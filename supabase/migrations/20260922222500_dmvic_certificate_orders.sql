-- DMVIC pay-per-certificate order and reconciliation foundation.
-- Customer/agency payments are collected by the platform. DMVIC settlement is
-- tracked separately from the certificate sale; there is no customer wallet.
...
-- Existing application authorization remains the authority. Authenticated users
-- can read certificate orders; writes are performed by authenticated server
-- functions which enforce role and state transitions.
