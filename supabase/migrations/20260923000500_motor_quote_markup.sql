-- Preserve insurer base price and agent/agency markup separately on motor policies/quotes.
-- The insurer price is immutable input; markup is additive and auditable.
...
alter table public.policies
  drop constraint if exists policies_markup_type_check;
alter table public.policies
  add constraint policies_markup_type_check
  check (markup_type is null or markup_type in ('fixed','percent'));

alter table public.policies
  drop constraint if exists policies_markup_nonnegative_check;
alter table public.policies
  add constraint policies_markup_nonnegative_check
  check (
...
alter table public.quotations
  drop constraint if exists quotations_markup_type_check;
alter table public.quotations
  add constraint quotations_markup_type_check
  check (markup_type is null or markup_type in ('fixed','percent'));

alter table public.quotations
  drop constraint if exists quotations_markup_nonnegative_check;
alter table public.quotations
  add constraint quotations_markup_nonnegative_check
  check (
    coalesce(insurer_base_premium,0) >= 0 and
    coalesce(markup_value,0) >= 0 and
    coalesce(markup_amount,0) >= 0 and
