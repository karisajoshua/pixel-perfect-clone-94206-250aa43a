    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {selectedBasePremium != null && <div className="rounded-md border p-3 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 rounded-md bg-muted/40 p-3 text-sm" aria-live="polite"><div><span className="text-muted-foreground">Base</span><div className="font-medium">KES {selectedBasePremium.toLocaleString()}</div></div><div><span className="text-muted-foreground">Markup</span><div className="font-medium">KES {Math.round(markupAmount*100)/100}</div></div><div><span className="text-muted-foreground">Client quote</span><div className="font-semibold">KES {Number(finalQuotedPremium??0).toLocaleString()}</div></div></div>
        </div>}
